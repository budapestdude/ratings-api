...
async function updateRapidBlitzRatings(players: Player[], ratingType: 'rapid' | 'blitz', dateStr: string) {
    ...
    try {
        ...
        // Only process top-rated players to speed up import
        const topPlayers = players
            .filter(p => p.fide_id && p.rating && p.rating > 2000)
            .sort((a, b) => (b.rating || 0) - (a.rating || 0))
            .slice(0, 10000); // Import top 10,000 players only for now
        ...
        for (const player of topPlayers) {
            ...
            if ((updatedCount + insertedCount) % 1000 === 0) {
...
async function main() {
    ...
    console.log('IMPORTING RECENT RAPID AND BLITZ RATINGS (Last 3 months)');
    console.log('='.repeat(60));
    console.log('');
    
    try {
        // Import last 3 months only
        const months = [
        ...
        // Check Carlsen's rapid/blitz history
        const carlsenHistory = await db.all(`
            SELECT rating_date, rapid_rating, blitz_rating 
            FROM ratings 
            WHERE fide_id = 1503014 
        ...
        console.log('\n🏆 Sample - Magnus Carlsen rapid/blitz history:');
        carlsenHistory.forEach(h => {
            console.log(`   ${h.rating_date}: Rapid ${h.rapid_rating || 'N/A'}, Blitz ${h.blitz_rating || 'N/A'}`);
        });
